...
def plot_2d(x,y,n,filename):
    ...
    ax.set_xlabel("Revised cases")
    ax.set_ylabel("Proportion with implant-ttr edge")
...
def main(args):
    ...
    group by registry_size;
    """.format(run_id = args[1])

    df = pd.read_sql_query(sql, engine)

    plot_2d(df["registry_size"], df["implant_ttr"], 1000, args[2])
